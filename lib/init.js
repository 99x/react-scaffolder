	...
	if(eslint === undefined) {
		...
		fs.mkdirp(projectPath, (error) => {
			...
			else
			{
				...
				Promise.all([copyCommonTemplates(projectName), copyTemplate(projectName, 'webpack.config.js'), copyTemplate(projectName, '.reactclirc'), copyTemplate(projectName, '.gitignore'), copyPackageJson(projectName)])
					.then((values) => {
						console.log(values);
						cb(true);
					})
