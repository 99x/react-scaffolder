 ...
 * @param {function} ddd - callback for status return
 */

init.prototype.initialize = function (projectName, gitrepository, eslint, cb, ddd) {
		...
		if (eslint === undefined) {
			...
			fs.mkdirp(projectPath, error => {
				if (error) {
					...
					clone(gitrepository, projectName + '/src', ddd);
