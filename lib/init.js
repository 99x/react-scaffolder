...
function init(projectName, eslint, cb) {
	if(eslint === undefined) {
		...
		fs.mkdirp(projectPath, (error) => {
			...
			else
			{
				...
				Promise.all([copyCommonTemplates(projectName), copyTemplate(projectName, 'webpack.config.js', false), copyTemplate(projectName, 'reactclirc', true), copyTemplate(projectName, 'gitignore', true), copyPackageJson(projectName)])
					.then((values) => {
						console.log(values);
						cb(true);
					})
	...
	{
		if(eslint) {
			Promise.all([copyCommonTemplates(projectName), copyTemplate(projectName, 'webpack.config.js', false), copyTemplate(projectName, 'gitignore', true), copyTemplate(projectName, 'eslintrc.json', true), copyPackageJson(projectName)])
				.then((values) => {
					console.log(values);
					cb(true);
				})
...
function copyTemplate(projectName, file, dotFile) {
	return new Promise(function(resolve, reject) {
		let destFile = null;
		if(dotFile) {
			destFile = '.' + file;
