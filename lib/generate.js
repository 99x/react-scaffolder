 ...
 */
generate.prototype.generateComponent = function(type, module, componentName, answers, answersInner, cb, onlyFile) {
	try {
      ...
      return;
		...
		if(componentName !== undefined) {
			...
			try {
				...
				fs.writeFileSync(path.join(process.cwd(), getSourceDirectory(), 'components', fileName + '.js'), __modFile, 'utf-8');
				cb(true);
			}
			catch(ex) {
				cb('error');
 ...
 * @param {string} onlyFile - option provided when creating component
 */
generate.prototype.createComponent = function(module, componentName, answers, answersInner, cb, onlyFile) {
	this.generateComponent(answers.componentType, module, componentName, answers, answersInner, cb, onlyFile);
}
