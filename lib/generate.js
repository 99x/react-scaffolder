...
generate.prototype.createConstants = function({ answers: { constants, path: _path }, folder, cb }) {
    ...
    fs.mkdirSync(path.join(process.cwd(), getSourceDirectory(), folder), 0755);
    fs.writeFileSync(path.join(process.cwd(), getSourceDirectory(), folder, 'constant.js'), output, 'utf-8');
    cb(true);
	} catch(e) {
		cb(e);
	}
