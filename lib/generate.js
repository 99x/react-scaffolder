 ...
 */
generate.prototype.generateComponent = function(type, module, componentName, answers, answersInner, cb, onlyFile = false) {
	try {
      ...
      return;
		...
		if(componentName !== undefined) {
			...
			try {
				...
				fs.writeFileSync(path.join(process.cwd(), getSourceDirectory(), 'components', directory, 'index.js'), __modFile, 'utf-8');
				cb(true);
			}
			catch(ex) {
				cb('error');
