...
var generate = function () {
   ...
   */
    ...
    */
			...
			var file = fs.readFileSync(path.join(__dirname, '..', 'templates/components/' + type + '-component.react.js'), 'utf-8');
			var re = /<name>/gi;
			try {
				if (componentName !== undefined) {
					...
					var _modFile2 = file.replace(re, module);
   ...
   * @param {function} cb - callback for status return
    ...
    */
			...
			var file = fs.readFileSync(path.join(__dirname, '..', 'templates/src/__tests__/app.js'), 'utf-8');
			try {
				if (testName !== undefined) {
