		...
		value: function generateComponent(type, module, componentName, answers, answersInner, cb) {
			try {
				if (componentName !== undefined) {
					var exists = fs.accessSync(path.join(process.cwd(), getSourceDirectory(), 'components', module, componentName), fs.F_OK);
					cb(componentName + '.react.js already exists');
					return;
				} else {
					var _exists = fs.accessSync(path.join(process.cwd(), getSourceDirectory(), 'components', module + '.react.js'), fs.F_OK);
					cb(componentName + '.react.js already exists');
					return;
				}
			} catch (e) {
				...
				if (componentName !== undefined) {
					try {
						if (answers.propTypes === 'no') {
							...
							var __modFile = this.createModFile(_modFile2, answersInner, componentName);
						...
						if (ex.syscall == 'open') {
							...
							var _modFile3 = _file.replace(re, componentName);
							var _modFile4 = this.createModFile(_modFile3, answersInner, componentName);
							fs.writeFileSync(path.join(process.cwd(), getSourceDirectory(), 'components', module, componentName + '.react.js'), _modFile4, 'utf-8');
							cb(true);
						}
					}
				} else {
					try {
						var _modFile5 = _file.replace(re, module);
						var _modFile6 = this.createModFile(_modFile5, answersInner, module);
						fs.writeFileSync(path.join(process.cwd(), getSourceDirectory(), 'components', module + '.react.js'), _modFile6, 'utf-8');
						cb(true);
					} catch (ex) {
						cb('error');
					}
		...
		value: function createTest(module, testName, cb) {
			try {
				if (testName !== undefined) {
					var existsModule = fs.accessSync(path.join(process.cwd(), getSourceDirectory(), '__tests__', module, testName), fs.F_OK);
					cb(componentName + '.js test file already exists');
					return;
				} else {
					var exists = fs.accessSync(path.join(process.cwd(), getSourceDirectory(), '__tests__', module + '.js'), fs.F_OK);
					cb(componentName + '.js test file already exists');
					return;
				}
			} catch (e) {
